...
宏是**写代码的代码**——编译时把简写展开成完整 Rust 代码。函数名后无 `!`，宏一定有 `!`。
...
macro_rules! my_macro {
    () => { println!("无参"); };
    ($val:expr) => { println!("有参: {}", $val); };
}
// 根据调用形式匹配不同规则
```

**注意**：多条规则之间用 `;` 分隔（最后一条可选）。
...
sum!(1, 2, 3);  // 展开为 total += 1; total += 2; total += 3;
```
...
宏定义默认是**私有的**，且**必须先定义后使用**。模块内的宏外部看不到：
...
// 三种导出方式：
// 方法一：#[macro_use]（放 module 上，一口气导出所有宏）
#[macro_use]
mod macros { ... }
...
|------|------|
| `#[macro_use]` | 模块级，批量导出 |
| `use macros::name` | 精确导入，推荐 |
| `#[macro_export]` | 跨 crate 可见，库常用 |
...
1. **`my_macro!()`** — 宏调用必须带 `!`
2. **先定义后使用** — 宏不像函数可以前向引用
3. **`#[macro_use]` / `use` / `#[macro_export]`** — 宏作用域控制的三种方式
4. **`$val:expr`** — 元变量 + 片段分类符，`$val` 在展开体中取值
5. **多规则用 `;` 分隔** — 最后一条的 `;` 可省略
...
Clippy 是 Rust 的官方 **lint 工具**，分析代码并给出改进建议，帮助捕捉常见错误和不符合惯用写法的代码。安装命令：`rustup component add clippy`。
...
1. **直接使用标准库常量** — 重复绑定多余
2. **`if let` vs `while let`** — 语义要精确，单次用 if，循环用 while
3. **避免 `unwrap` 裸奔** — 不确定值时用 `if let` 或 `match`
4. **变量类型和命名要一致** — 别叫 vec 却是 `()`
5. **`std::mem::swap`** — 替代手动 temp 变量交换
...
Rust 提供两种范围类型，底层是不同的结构体：

| 语法 | 类型名 | 区间 | `contains` 逻辑 | 示例 |
|------|--------|------|----------------|------|
| `a..b` | `std::ops::Range` | `[a, b)` 左闭右开 | `a <= x < b` | `(0..5).contains(&5)` → `false` |
| `a..=b` | `std::ops::RangeInclusive` | `[a, b]` 双闭 | `a <= x <= b` | `(0..=5).contains(&5)` → `true` |

还有三个不那么常用的：
- `..b` = `RangeTo`，`(..5).contains(&3)` = `3 < 5` → `true`
- `a..` = `RangeFrom`，`(3..).contains(&100)` = `3 <= 100` → `true` （无始无终的无限范围）
- `..` = `RangeFull`，表示一切

### `contains()` — 判断值是否在范围内

`contains()` 是范围最直接的检测方法，传引用调用：
...
`contains` 要求类型实现 `PartialOrd` + `PartialEq`，以下类型都满足：

```rust
(1..=10).contains(&5);           // i32     ✅
(1.0..=10.0).contains(&5.5);     // f64     ✅
('a'..='z').contains(&'x');      // char    ✅
// ⚠️ String/&str 不实现 PartialOrd，不能用范围
```

### `for` 循环中的 `..` vs `..=`

```rust
for i in 0..5 {    // i = 0, 1, 2, 3, 4（不包含 5）
    println!("{i}");
}

for i in 0..=5 {   // i = 0, 1, 2, 3, 4, 5（包含 5）
    println!("{i}");
}
```

`for` 循环中 `..` 和 `..=` 最直观的区别就是：一个不含终点，一个含终点。

### Range 是迭代器

`Range` 实现了 `Iterator`，可以直接放进 `for` 或使用迭代器方法：
...
注意：`RangeInclusive`（`..=`）也可以 `rev()`，但端点值 `=` 包含会被尊重。
...
let inc_range: std::ops::RangeInclusive<i32> = 0..=10;
...
| 场景 | 推荐语法 | 原因 |
|------|---------|------|
| 判断值是否在范围内 | `(low..=high).contains(&x)` | 语义直接，无分支 |
| `for` 遍历 n 次 | `0..n` | 不含终点，符合索引习惯（0..len） |
| `for` 遍历 1 到 n | `1..=n` | 含终点，语义明确 |
| 字符范围判断 | `('a'..='z').contains(&c)` | 比 `c >= 'a' && c <= 'z'` 更简洁 |
| 切片索引 | `&arr[0..5]` | 切片只接受 `..` 语法，不支持 `..=` |

### `..=` 的独特地位

`..=` 是 `RangeInclusive` 的语法糖，**唯一**能做到"包含终点"的范围写法。在以下场景尤为重要：

1. **判断是否在范围内**：`(0..=100).contains(&score)` — 如果能得分 100，必须用 `..=`
2. **字符类匹配**：`('a'..='z').contains(&ch)` — 包含 'z'
3. **match 守卫中的条件**：无需写 `x >= low && x <= high`
4. **遍历包含终点的序列**：`for i in 1..=n` 比 `for i in 1..n+1` 更能表达意图
