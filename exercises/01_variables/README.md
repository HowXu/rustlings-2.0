...
```rust
let x = 42;        // x 是一个装着 42 的盒子（在栈上某地址）
let r = &x;        // r 是一张写着"盒子地址"的纸条（引用）
let y = *r;        // *r = 顺着纸条找到盒子，取出里面的 42
```
...
// 2. 读取引用背后的值（简单类型会自动解引用，可省略）
let b = 10;
let r = &b;
println!("{}", r);    // 可以，println 自动解引用
println!("{}", *r);   // 也可以显式写

// 3. 类型转换时需要解引用
let x: usize = 5;
let r = &x;
let y: i32 = *r as i32;  // 先解引用取出 usize，再 as 转 i32
```